...
> ⚠️ 定位優先序：**名稱 > component key > 現值 id**。`clone()` 只能同檔；跨檔用 component key 重建。

### ⭐ Path C — 版型模板元件 keys（import→detach→retext）｜源檔 `VTxr1LgruTEL86yinALPw0`
component set `[test]Template_agent_pages`（key `acd4b964a532223cc666a1fe0d2572cfd90f4757`）4 variant，含 chrome：
full-page `f860d871f076be9e0bd4bbc4df2d7816020f19c5`｜side-panel `630133a62a582f61c170ec64d3d0110d2ff74072`｜full-page+history `147c4a1a6dfde1eaab26e51de1d1d2db5c09fdc2`｜side-panel+history `6272eb91b96f4185339c70b37fb15d719bbcac12`
> 需已發佈才能跨檔 import；detach 保留巢狀 instance（已驗證）；turn 數固定→detach 後 clone 補齊。`[test]` 前綴=測試中。

### 頁面分佈（2026-06 實測）
| 內容 | 頁面 | 頁 id |
|---|---|---|
| 元件 spec 區（①–⑥）+ 標籤 | `Agent` | `0:1` |
| 版型範例（full-page / side-panel / history）+ 常用目的地 frame | `agent #1` | `13:960` |

### Component keys（最穩定；`importComponentByKeyAsync` 重建用）
| 元件 | key |
|---|---|
...
### 版型範例（clone 來源，先按名稱找）
| 範例 | frame 名稱（穩定） | 現值 id（會變） |
|---|---|---|
| Full-page | `full-page chat ( with AI agent)` | `119:13019` |
| Side-panel | `side -panel chat ( with ai agent)` | `119:11980` |
| Full-page + history | `full-page chat ( history)` | `119:15795` |
| Side-panel + history | `side -panel chat ( history)` | `119:15787` |

### 元件 spec 區（純元件、無 app 外框）
Chatroom `4:3279`①｜Chat history(Side) `6:3200`②｜Response/Turn `6:3710`③｜User msg `6:4397`④｜AI msg `6:4587`⑤｜Chat input `6:4856`⑥｜Chat history(Full) 主檔 `119:16575`/實例 `119:17097`
